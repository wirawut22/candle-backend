                        ...
                        <div class="full">
                            <div class="row">
                                <div class="heading_main text_align_left">
                                    <h2><span>{{placePost.topic}}</span></h2>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-12" id="other-detail">
                                    <i class="fas fa-user" title="ผู้เขียน"></i> ผู้เขียน : {{placePost.author}} | 
                                    <i class="fas fa-calendar-alt" title="อัพเดทล่าสุด"></i> อัพเดทล่าสุด : {{placePost.update_dttm | moment}}
                                </div>
                            </div>
                            <div class="row mt-3 mb-3">  
                                <div class="input-group" v-if="placePost">  
                                        <div class="col-12 text-center">
                                            <img v-if="!imageActive && placePost.image.length==0" src="/base/images/lazy.jpg" class="product-image" max-height="500"/>
                                            <img v-if="!imageActive && placePost.image.length>0" :src="placePost.image[0]" class="product-image" max-height="500"/>
                                            <img v-if="imageActive" :src="imageActive" class="product-image" max-height="500"/>
                                        </div>
                                        <div class="col-12 product-image-thumbs"  >
                                            <div v-for="item in placePost.image" :key="item" v-bind:index="index" class="product-image-thumb" v-on:click="setActiveImage(item)">
                            ...
                            <div class="row">
                                <div class="full view">
                                    <div v-html="placePost.description1"></div>
                                </div> 
                            </div>
                        </div>
                    </div> 
